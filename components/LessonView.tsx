...
import { ArrowLeft, Clock, AlertTriangle, ExternalLink, CheckCircle, XCircle, Trophy, BookOpen, Play } from 'lucide-react';
...
export const LessonView: React.FC<Props> = ({ 
  ...
  if ((content.type === 'MCQ_ANALYSIS' || content.type === 'MCQ_SIMPLE') && content.mcqData) {
      ...
      }, [content.mcqData, chapter.id]);

      // --- SAVE PROGRESS LOGIC ---
      useEffect(() => {
          if (Object.keys(mcqState).length > 0) {
              const key = `nst_mcq_progress_${chapter.id}`;
      ...
      }, [mcqState, batchIndex, chapter.id, localMcqData]);
      ...
      // If showing results (Analysis), show ALL questions up to current batch. Otherwise show current batch only.
      const currentBatchData = showResults 
          ? displayData.slice(0, (batchIndex + 1) * BATCH_SIZE) 
          : displayData.slice(batchIndex * BATCH_SIZE, (batchIndex + 1) * BATCH_SIZE);
      ...
      const handleGateAction = (action: 'UNLOCK' | 'SKIP' | 'SUBMIT') => {
          if (action === 'UNLOCK') {
              setShowResults(true); // Show Analysis
              setShowGate(false);
          ...
          <div className="flex flex-col h-full bg-slate-50 animate-in fade-in relative">
               ...
               <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                   ...
                   <div className="text-right">
                       ...
                       {showResults ? (
                           <span className="text-xs font-bold text-green-600">Final Score: {score}/{displayData.length}</span>
                       ) : (
                           <span className="text-xs text-slate-400">Batch {batchIndex + 1} • {Object.keys(mcqState).length}/{displayData.length} Total</span>
                       )}
                   </div>
               </div>
                   ...
                   {currentBatchData.map((q, localIdx) => {
                       ...
                       return (
                           <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md">
                               ...
                               {/* Show Explanation Only in Analysis Mode (showResults) */}
                               {showResults && (
                                   <div className="mt-4 pt-4 border-t border-slate-100 animate-in slide-in-from-top-2">
                                       ...
                                       {q.explanation && q.explanation !== "Answer Key Provided" && (
                                            <div className="text-slate-600 text-sm bg-slate-50 p-3 rounded-lg border border-slate-200 mt-2">
                                                <span className="font-bold text-slate-800 block text-xs uppercase mb-2">Explanation:</span>
                                                {q.explanation.split('\n').map((line, i) => (
                                                    <p key={i} className={`mb-2 ${line.trim().startsWith('•') || line.trim().match(/^\d+\./) ? 'pl-2' : ''}`}>
                                                        {line}
                                                    </p>
                                                ))}
                                            </div>
               ...
               {!showResults ? (
                   <div className="p-4 bg-white border-t border-slate-200 sticky bottom-0 z-10 flex justify-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] gap-4">
                       {hasMore ? (
                           <button 
                               onClick={() => setShowGate(true)}
                               className="bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-10 rounded-xl shadow-lg transition-all active:scale-95 flex items-center gap-2"
                           >
                               Next Batch <ArrowLeft size={18} className="rotate-180" />
                           </button>
                       ) : (
                           <button 
                   ...
                   <div className="p-4 bg-white border-t border-slate-200 sticky bottom-0 z-10 flex justify-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] gap-4">
                       <button onClick={onBack} className="bg-slate-100 text-slate-600 font-bold py-3 px-8 rounded-xl hover:bg-slate-200">
                           Exit
                       </button>
                       {hasMore && (
                           <button 
                               onClick={() => { setShowResults(false); setBatchIndex(prev => prev + 1); document.querySelector('.mcq-container')?.scrollTo(0,0); }} 
                               className="bg-blue-600 text-white font-bold py-3 px-8 rounded-xl shadow-lg hover:bg-blue-700 flex items-center gap-2"
                           >
                               Continue Next 50 <ArrowLeft size={18} className="rotate-180" />
                           </button>
                       )}
                   </div>
               )}
               ...
               {showGate && (
                   <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in">
                       <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl animate-in zoom-in">
                           ...
                           <h3 className="text-2xl font-black text-slate-800 mb-2">Batch Completed!</h3>
                           <p className="text-slate-500 text-sm mb-8">
                               You have finished 50 Questions. Want to see the detailed analysis report?
                           </p>
                           
                           <div className="space-y-3">
                               <button 
                                   onClick={() => handleGateAction('UNLOCK')}
                               ...
                               >
                                   <BookOpen size={20} /> View Analysis (Review)
                               </button>
                               
                               <button 
                                   onClick={() => handleGateAction('SKIP')}
                               ...
                               >
                                   Next 50 Questions <ArrowLeft size={18} className="rotate-180" />
                               </button>

                               <button 
                                   onClick={() => handleGateAction('SUBMIT')}
