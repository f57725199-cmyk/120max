...
import { ToggleLeft, ToggleRight, Trees } from 'lucide-react';
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate }) => {
  ...
  const handleSeasonNavigate = async (subjectName: string, chapterName: string) => {
      // 1. Find Subject
      ...
      onTabChange('COURSES'); // Switch tab so the view renders correct area
      
      try {
          // Temporarily set view to Subjects to show loading if needed, or just let overlay handle it
          setSelectedSubject(subject);
          setContentViewStep('CHAPTERS');
          setLoadingChapters(true);

          const fetchedChapters = await fetchChapters(user.board || 'CBSE', user.classLevel || '10', user.stream || 'Science', subject, 'English');
          setChapters(fetchedChapters);
          
          // 3. Find Chapter (Fuzzy Match)
          const chapter = fetchedChapters.find(c => c.title.toLowerCase().includes(chapterName.toLowerCase()) || chapterName.toLowerCase().includes(c.title.toLowerCase()));
          ...
          if (chapter) {
              ...
              // If not found, show the chapter list so user can find it manually
              // alert(`Chapter "${chapterName}" content coming soon! Opening subject...`);
              // Already set to CHAPTERS above
          }
      } catch (e) {
          console.error(e);
          setLoadingChapters(false);
  ...
  const renderContentSection = (type: 'VIDEO' | 'PDF' | 'MCQ') => {
      ...
      if (contentViewStep === 'PLAYER' && selectedChapter && selectedSubject) {
          if (type === 'VIDEO') {
            ...
            return <McqView chapter={selectedChapter} subject={selectedSubject} user={user} board={user.board || 'CBSE'} classLevel={user.classLevel || '10'} stream={user.stream || null} onBack={handlePlayerBack} onUpdateUser={handleUserUpdate} />;
          }
      }
              ...
              <div className="space-y-6 pb-24">
                  ...
                  <div className="flex items-center justify-between px-1">
                      <h3 className="font-bold text-slate-800 flex items-center gap-2">
                          <Trees size={18} className={isSeasonMode ? "text-emerald-600" : "text-slate-400"} /> 
                          Season Plan
                      </h3>
                      <button 
                        onClick={() => setIsSeasonMode(!isSeasonMode)}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm border ${isSeasonMode ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-white text-slate-500 border-slate-200'}`}
                      >
                          {isSeasonMode ? (
                              <>Active <ToggleRight size={18} className="text-emerald-600" /></>
                          ) : (
                              <>Default <ToggleLeft size={18} className="text-slate-400" /></>
                          )}
                      </button>
                  </div>
                  ...
                  {isSeasonMode ? (
                      <div className="h-[500px]">
                          <SyllabusTreeView 
                              classLevel={user.classLevel || '10'} 
                              onNavigateToChapter={handleSeasonNavigate} 
                              progress={user.syllabusProgress}
                          />
                      </div>
                  ) : (
                  /* DEFAULT VIEW (More Services) */
                  <div>
